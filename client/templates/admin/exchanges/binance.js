Template.binance.onCreated(function () {
    ...
    Meteor.call('binance.loadMarkets', function (error, data) {
    ...
    this.loadSelectedMarketInfo = () => {
        Meteor.call('binance.fetchTrades', this.selectedMarket.get(), function (error, data) {
            handleFetchTrades(error, data);
        });
        Meteor.call('binance.fetchTicker', this.selectedMarket.get(), function (error, data) {
            handleFetchTicker(error, data);
        });
        Meteor.call('binance.fetchOrderBook', this.selectedMarket.get(), function (error, data) {
            handleOrderBook(error, data);
        });

        Meteor.call('binance.fetchBalance', function (error, data) {
            console.log(data);
            handleBalance(error, data);
        });
        // Meteor.call('binance.fetchMyTrades', this.selectedMarket.get(), function (error, data) {
        //     console.log(data);
...
Template.binance.events({
    ...
    'click #btnConfigApi': function (event, instance) {
        Meteor.call('exchanges.registerApi', $('#fundo_id').val(), 'binance', $('#apikey').val(), $('#apisecret').val(), function(error, data){
            handleGeneralError(error);
            if (error == null){
                location.reload();
            }
