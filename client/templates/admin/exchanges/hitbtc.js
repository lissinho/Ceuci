Template.hitbtc.onCreated(function () {
    ...
    Meteor.call('hitbtc.loadMarkets', function (error, data) {
    ...
    this.loadSelectedMarketInfo = () => {
        Meteor.call('hitbtc.fetchTrades', this.selectedMarket.get(), function (error, data) {
            handleFetchTrades(error, data);
        });
        Meteor.call('hitbtc.fetchTicker', this.selectedMarket.get(), function (error, data) {
            handleFetchTicker(error, data);
        });
        Meteor.call('hitbtc.fetchOrderBook', this.selectedMarket.get(), function (error, data) {
            handleOrderBook(error, data);
        });

        Meteor.call('hitbtc.fetchBalance', function (error, data) {
           console.log(data);
           handleBalance(error, data);
        });
        //Meteor.call('hitbtc.fetchMyTrades', this.selectedMarket.get(), function (error, data) {
        //    console.log(data);
...
Template.hitbtc.events({
    ...
    'click #btnConfigApi': function (event, instance) {
        Meteor.call('exchanges.registerApi', $('#fundo_id').val(), 'hitbtc', $('#apikey').val(), $('#apisecret').val(), function(error, data){
            handleGeneralError(error);
            if (error == null){
                location.reload();
            }
