Template.mercadobitcoin.onCreated(function () {
    ...
    Meteor.call('mercadobitcoin.loadMarkets', function (error, data) {
    ...
    this.loadSelectedMarketInfo = () => {
        Meteor.call('mercadobitcoin.fetchTrades', this.selectedMarket.get(), function (error, data) {
            handleFetchTrades(error, data);
        });
        Meteor.call('mercadobitcoin.fetchTicker', this.selectedMarket.get(), function (error, data) {
            handleFetchTicker(error, data);
        });
        Meteor.call('mercadobitcoin.fetchOrderBook', this.selectedMarket.get(), function (error, data) {
            handleOrderBook(error, data);
        });

        Meteor.call('mercadobitcoin.fetchBalance', function (error, data) {
           console.log(data);
           handleBalance(error, data);
        });
        //Meteor.call('mercadobitcoin.fetchMyTrades', this.selectedMarket.get(), function (error, data) {
        //    console.log(data);
...
Template.mercadobitcoin.helpers({
    markets() {
...
Template.mercadobitcoin.events({
    ...
    'click #btnConfigApi': function (event, instance) {
        Meteor.call('exchanges.registerApi', $('#fundo_id').val(), 'mercadobitcoin', $('#apikey').val(), $('#apisecret').val(), function(error, data){
            handleGeneralError(error);
            if (error == null){
                location.reload();
            }
